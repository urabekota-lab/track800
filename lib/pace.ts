 ...
 * 練習条件に応じてタイムを割り引く。
 *
 * ここは以前 1.5% 引き程度の弱い係数にしていたが、明確に誤りだった。
 * インターバルや連続走は**設定ペースを守って走る**練習で、全力ではない。
 * それをほぼ全力として扱っていたため、練習を記録するほど推定が遅くなっていた。
 *
 * 実勢の目安: 2:00 の選手は 600m×4 を 1:32〜1:38 で回すが、単発全力の 600m は
 * 1:25〜1:26。比は約 0.90。完全回復のレペならもう少し全力に近い。
 *
 * 経験則ベースの係数であり、厳密な生理学的定数ではない。
 * 本数やレストの長さで実際は上下するので、これに頼る推定は信頼度を下げている。
 */
const EFFORT_FACTOR: Record<Effort, number> = {
  trial: 1.0,        // レース・タイムトライアル（そのまま使える）
  repetition: 0.97,  // 完全回復レペティション（ほぼ全力）
  interval: 0.90,    // 短〜中レストのインターバル（設定ペース）
  continuous: 0.85,  // 連続走・変化走・ビルドアップ（明確に抑えて走る）
}
